...
contract PetShop {
    ...
    function buyAKitten() public payable returns (uint256) {
        require(
            msg.value == 0.5 ether,
            "You forgot to pay 0.5 ether for the cat."
        );
...
contract KittyShelter {
    ...
    mapping(uint256 => StoredCatInfo) catStorage; // catId -> StoredCatInfo

    struct StoredCatInfo {
        address previousOwner;
        uint256 releaseTimestamp;
    }

    event CatStored(address owner, uint256 catId, uint256 releaseTimestamp);
    event CatReleased(address owner, uint256 catId);

    constructor(address _kittenRegistry) {
        registryAddress = _kittenRegistry;
    }

    function storeKitty(uint256 catId, uint256 time) public {
        address owner = KittenRegistry(registryAddress).ownerOf(catId);
        require(owner == msg.sender, "You don't have permission");

        KittenRegistry(registryAddress).transferFrom(
            msg.sender,
            address(this),
            catId
        );

        catStorage[catId] = StoredCatInfo(msg.sender, block.timestamp + time);

        emit CatStored(owner, catId, block.timestamp + time);
    }

    function retrieveKitty(uint256 catId) public {
        address owner = KittenRegistry(registryAddress).ownerOf(catId);

        require(owner == address(this), "Cat is not owned by contract");
        require(
            catStorage[catId].previousOwner == msg.sender,
            "You don't have permission"
        );
        require(
            catStorage[catId].releaseTimestamp <= block.timestamp,
            "Cat is still locked"
        );
        ...
        delete catStorage[catId];

        emit CatReleased(owner, catId);
    }
}
